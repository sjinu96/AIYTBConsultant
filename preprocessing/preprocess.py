...
class Preprocessor:
    ...
    def _fit_transform(self, raw: pd.DataFrame) -> pd.DataFrame:
        '''fit and transform raw dat
        
        '''
        result = raw.copy()

        result = self._n_comment_to_float(result) # n_comment 변수 실수화
        result = self._str_to_datetype(result) # 데이터 로딩 과정에서 문자열로 불러진 datetype을 기존 datetype으로
        result = self._add_n_hashtag(result) # 해시태그 갯수
        ...
        result = self._add_diff(result, features, new_name) # 구독자 변롸량 컬럼 추가
    ...
    def _n_comment_to_float(self,result):
        '''make "n_comment" feature numerical treating "댓글 사용 중지" and NaNs'''
